...
> **Correction to the explanation** (the observation stands): the `0.6ⁿ` reading is wrong, because a 40%-of-own-troops cap can never reach 0 — it only asymptotes toward 1. Units are removed from a tactical battle by a separate **rout check** (`FUN_00438fb0`), which fires when a unit's troops fall below its type's standard battalion size ÷ 25, or on a morale test. Heavy cavalry's threshold is the lowest in the game (2,500 ÷ 25 = **100 troops**), so small heavy-cavalry units do collapse first — for that reason rather than through compounding attrition. See [battle-replayed-rout-mechanic-and-combat-constants.md](battle-replayed-rout-mechanic-and-combat-constants.md).
