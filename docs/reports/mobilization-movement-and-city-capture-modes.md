...
> **Correction (2026-09-19): only `0 money` is a creation value.** `FUN_00449f08`, the army-create routine, is now decompiled: a new army starts with **0 supplies, 0 money, 0 moves (1 for an AI nation) and morale 59**. The `8 moves` observed here is the weekly tick's recomputation `10 − min(5, ⌊43,000/20,000⌋)`; the `410 tons` is a resupply, exactly as this report's own next section suspects when it warns that multi-turn pairs conflate transfer, movement and production; and the `morale 2` is the `+8` **covered map cell** (terrain code 2), not morale — the real morale word in `autumn_3` reads `60`, the created `59` plus the tick's `+1`. Everything else in this section holds, and the rest of the pair confirms the transfer routine clause by clause — including that **Army 0 filled to exactly 20 units before the overflow created Army 14**, that Army 14 sits at `(102, 44)` = Rome `(101, 43)` plus `(+1, +1)`, the last cell of the placement scan, and that all eleven mobilized units carry quality `6` = `state 24 / 4` while the state-`8` slot was correctly left behind. See [decompiled-mobilization-and-mercenary-restock.md](decompiled-mobilization-and-mercenary-restock.md) §7.

A freshly created army (Army 14) has a recognizable fingerprint: round troop count, **410 tons supply**, **0 money**, **8 moves**, **morale 2** (see the correction above). Fortification percentage (78%) did not change even though the garrison troop count collapsed, confirming it is a separate, non-troop-derived stat from the parenthetical troop total, consistent with [prior findings](city-units-army-transfer-and-mercenaries.md).
...
> **Confirmed in code (2026-09-19), and the reason is now known.** `FUN_0044a4e0` never touches `+0x442`. The field is raised by the **recruitment order** (`min(100, mobilized + 1 + troops × 1000 / wealth)`) and lowered by cancelling one; mobilizing a completed order only moves troops that were already counted. This paragraph's inference was right and is now `[confirmed]`. See [decompiled-mobilization-and-mercenary-restock.md](decompiled-mobilization-and-mercenary-restock.md) §5.
