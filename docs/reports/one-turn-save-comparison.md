...
> **Correction (2026-09-14):** it is a separator. At the end of every round tick, `FUN_004514EC` writes a slot containing a single space (`" "`), then the `Week  N      Season      YYYBC` header, as two ordinary news entries. `Week 1      Spring      270 BC`, with a space before "BC", is one of 27 lines seeded from the DAT, not a game-written header. See [`news-log-format-and-messages.md`](news-log-format-and-messages.md).
