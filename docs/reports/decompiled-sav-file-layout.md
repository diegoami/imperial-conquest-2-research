...
> **Correction (2026-09-14):** see [`news-log-format-and-messages.md`](news-log-format-and-messages.md).
> - **The gap is closed.** The 3,042 bytes were measured from the end of the nation table to the *start* of the trailer, and every save measured had a full log. So 3,042 = 600 + 2 + 40 × 61 exactly. Subtracting the 55-byte tail again was the error.
> - **The tail is fully named from the save code**, and checked on all 54 saves: 32 turn order; 4 pending offer; 2 current nation; then 2+2+2+2 = turn-order index, week, year BC and season (the "unidentified" fields); then 8 bytes of main-window geometry (UI state, not calendar); then the 1-byte battle flag.
> - Week, year and season equal the last news header in 54 of 54 saves.
