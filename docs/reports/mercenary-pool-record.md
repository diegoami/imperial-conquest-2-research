...
> **Update (2026-09-24).** **`Label` is answered.** It indexes 52 ethnic mercenary names; the table is in the DAT at `0x1F8C6` (20-byte NUL-terminated strings: `0 Regular`, … **`11 Gallic`**, … **`35 Egyptian`**, …). The two values this report could not match to a nation, Felsina's `11` and Alexandria's `35`, are exactly the hired unit's name and the listed offer's name. **The position is also a hiring rule, not just a label.** The player can hire only with an army at Chebyshev distance 1 from the offer's city. The AI hires every offer within distance 4. See [decompiled-mercenary-offer-list-and-position.md](decompiled-mercenary-offer-list-and-position.md).
