...
- **The storm-damage predicates.** `FUN_004494e4` (the test that doubles damage, inferred here as "away from friendly coast") and `fleet[+24] == 1` (which triples it) are inferred from magnitudes, not decompiled. The Cartago series is consistent with the doubling branch being active throughout but cannot prove which predicate selected it.
  > **Correction (2026-09-25):** both predicates are now decompiled; see [decompiled-map-code1-overlay.md](decompiled-map-code1-overlay.md). `fleet[+24]` is the fleet's covered map cell, and `== 1` means the fleet sits on **rough sea**, the weekly weather overlay. `FUN_004494e4(owner, xy)` returns the index of a city **owned by the fleet's own nation** within the clamped 3×3, or −1. So the doubling branch means "not adjacent to one of its own cities"; it does not test "away from friendly coast" in general.
- **Auto-resupply.** Several armies and fleets hold a constant supply percentage for many turns against the consumption rule — e.g. an army at 80 % then 95 % for nine consecutive turns — so something resupplies units near friendly cities outside the `TAFSupply` dialog path. Not traced. It never touched the Thracian army, which is why that series is clean. **(Traced 2026-09-14:** `FUN_0044F6D8`, army, and `FUN_0044F7E4`, fleet. The AI calls them each turn for every non-hostile city within 4 tiles, and a human army calls `FUN_0044F6D8` when its move ends against such a city. They fill to `troops div 100` and `ships × 8`. See [`supply-capacity-rounding.md`](supply-capacity-rounding.md).**)**
