...
3. ~~The army `moves` recomputation after a unit-composition change is still an open formula.~~ **Answered 2026-09-18: there is no recomputation after a composition change.** The weekly maximum, `10 - min(5, floor(troops / 20000))` minus 1 below 10 % supply, is written once per week by the global tick (`FUN_004514EC`, `0x00451651`) and is not re-derived when units move between armies. The Roman army at `(100, 42)` proves it directly: 48,173 troops with `moves = 8` in `11_supply.sav` (the formula's value), then 63,173 troops after the 15,000-troop city transfer with `moves` still `8` in `12_mac.sav`/`12_rom_a.sav`, where the formula would give 7. Those are the only 2 of 627 army records in the corpus that exceed the formula. See [army-moves-field-signed-and-the-ffff-underflow.md](army-moves-field-signed-and-the-ffff-underflow.md).
