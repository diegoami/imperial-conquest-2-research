...
| **Split army** | `TUnitMap_SplitArmy` `0x0044755C` | Needs ≥ 2 units. Opens `TSplitArmyUnit`. `FUN_00449F08` creates the new record: cap **198 armies** total (`< 0xC6`), moves `0` for a human nation and `1` for an AI one, supplies/money `0`, morale `59`. **Those are the values `TSplitArmyUnit` *opens* with, not necessarily the committed ones**: the dialog is a two-pane transfer screen with `Transfer`/`Disband` per side and supply/money spinners (10s and 100s steppers), so the player rebalances both before `OK`. The morale `59` and exact troop/unit conservation are confirmed against a real split in [pending-offer-block-army-split-and-naupactus.md](pending-offer-block-army-split-and-naupactus.md). |
...
> **User testimony (2026-09-18), corroborating the provider set.** Asked about fleet resupply while the
> reimplementation's naval task was in review, the user stated from play experience: *"fleet can supply
> from and to armies, towns and other fleets."* That matches this section's own findings from three
> directions — `TUnitMap_SupplyFleet` opening `TAFSupply` **against a city or another fleet**,
> `TUnitMap_SupplyArmy` opening the same dialog, and `TAFSupply_FindProviders` offering the nation's own
> fleets within one tile alongside cities. It is recorded because the decompilation alone left it
