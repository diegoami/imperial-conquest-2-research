...
>
> **Addition (2026-09-18):** `+6` is a **signed** 16-bit count, not unsigned — every comparison in the binary is `JLE`/`JGE` and the army panel widens it with `MOVSX`. Its weekly value is `10 − min(5, ⌊totalTroops / 20000⌋)`, minus `1` below 10 % supply, which reproduces this report's `8` for 48,173 troops exactly. One record in the corpus reads `0xFFFF`; decoded unsigned that prints as `65535`, but it is `−1` from an underflow bug in the original. See [army-moves-field-signed-and-the-ffff-underflow.md](army-moves-field-signed-and-the-ffff-underflow.md).
...
> **Correction (2026-09-19).** `+0` within a unit slot is **not unknown**: it is the unit's **origin label** — `0` for a nationally recruited unit (written by the mobilization routine `FUN_0044a4e0`), and the mercenary offer's `Label` for a hired one, which is what makes a hire display as "Gallic" instead of taking a battalion number. The naming routine `FUN_0044a218` skips every unit whose `+0` is non-zero when assigning ordinals. See [decompiled-mobilization-and-mercenary-restock.md](decompiled-mobilization-and-mercenary-restock.md) §3.

Each unit slot contains an origin label at `+0` (see the correction above), a type code at `+2`, troop count at `+4`, quality code at `+6`, and a 24-byte name area beginning at `+8`. A slot with zero troops can still retain a name or filler bytes, so zero-troop slots are not shown as active units. The observed names are NUL-terminated ASCII. This screenshot identifies type codes `0` as light infantry, `1` as heavy infantry, `3` as light cavalry, and `4` as heavy cavalry. [Later recruitment screenshots](rome-city-recruitment-and-nations.md) identify code `2` as archers. Quality codes `6`, `7`, `8`, and `9` map to **average**, **good**, **very good**, and **elite** respectively. A later [15,000-soldier transfer](city-units-army-transfer-and-mercenaries.md) supports code `5` as **poor**.
