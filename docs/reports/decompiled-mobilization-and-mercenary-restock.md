...
- **What `Label` means** — section 3 identifies the table it indexes (`0x0049CC94`, 20-byte strings) but the table is BSS, loaded from the DAT, and its DAT offset was not located this pass. The strings are readable the same way `0x1F6CA` was.
  ...
  **Update (2026-09-24):** the DAT offsets are now located: the name table is at DAT `0x1F8C6` and the 251-record template-plus-pool table at `0x1FCD6`, both read by the DAT loader `FUN_004481A0`. The label strings are listed in [decompiled-mercenary-offer-list-and-position.md](decompiled-mercenary-offer-list-and-position.md) §5. That section also refines §6: all 201 template positions are city tiles, and `rand(200)` never draws template `200`, so only 200 templates are live.
