...
| Supplied `11_ptol.sav` | `185850942e71a713d6fbf9f785c3640ccc6507fb418e409b8ce34b038775ba86` | Ptolemaic human player added; Week 7 |
| Supplied `12_ptol.sav` | `72b49b0b74f95ca443e6f7a9339199bceaccb283d6c0adca4506e5014352a1e7` | After Roman end turn; Week 9 |
| Supplied `12_ptol_b.sav` | `da0d1e029684a776a484a4575977c75f3c1a8809d468bd88350b124eff4264ff` | Two Ptolemaic armies moved toward Masada and Alexandria |
| Supplied `12_rom_a.sav` | `b58e551487833a6b0fadf25d0b9be0b461299662b10c37090748b9791813310d` | Post-transfer Roman army; this is the filename present on disk |
...
Five further `11_supply` screenshots show Rome's city information, its recruitment list, and an ordered nation icon/name key. Rome's city record contains **1,731** supplies, **78** fortification percent, **181** population thousands, and **317** tribute talents, matching the panel exactly. A 40-slot city-unit section contains twelve Rome entries whose type codes and troop quantities match the displayed list; type code `2` is now identified as archers. That section is embedded in Rome's 1,172-byte nation record, and all sixteen nations have equivalent slots. The positional icon/name key resolves all sixteen nation identifiers and corrects the viewer colors for Celtiberia, Armenia, and Media. See [Rome city, recruitment, and nations](reports/rome-city-recruitment-and-nations.md). Loyalty thresholds and the city-unit state word remain unresolved.

The controlled `11_supply.sav` → `11_ptol.sav` pair changes just three bytes, all in the Ptolemaic nation record. Its `+0x490` field changes `0 → 1` when the human player is added; Rome already has `1`. Nation records also reproduce the Rome and Carthage panels' leaders, capitals, city counts, tax rates, Rome's mobilized percentage and treasury, and current human status. The displayed national population equals three times the sum of owned city population fields for those two nations, a candidate aggregation rule. The Roman army information panel matches its moves, supplies, money, terrain, and five troop-class totals; morale value `9` corresponds to “very high” in this sample. In `12_ptol.sav`, trailer fields reproduce **Week 9 Summer 270 BC** and the active **Ptolemaic** turn. The end-turn pair also changes eight map cells, eighteen city supply fields, and many nation and entity fields. See [Ptolemaic player and Week 9 analysis](reports/ptolemaic-player-and-week9.md) for offsets and limits.

The next three-save sequence resolves the city's parenthetical troop count. Rome's twelve city-unit slots sum to **85,000**. Transferring one 15,000 light-infantry unit to its army leaves **70,000** in eleven slots and raises the army from **48,173 to 63,173** troops, matching the Rome and army screenshots. Masada provides a second city check: six units sum to **49,800**, exactly its displayed parenthetical count despite a population of only 10,000 and fortification of 98%. The two Ptolemaic army moves match changed coordinates and remaining-moves fields. The transferred army unit's quality code `5` supports “poor.” Alexandria's **Egyptian light infantry, 9,056, good** mercenary matches a separate candidate 12-byte record just after the nation table; its complete table boundaries remain unknown. See [city units, army transfer, and mercenaries](reports/city-units-army-transfer-and-mercenaries.md).
